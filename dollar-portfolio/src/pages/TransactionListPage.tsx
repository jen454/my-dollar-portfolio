  ...
  return (
    <main style={{ display: "flex", flexDirection: "column", minHeight: "100vh", background: colors.grey50 }}>
      ...
      <div style={{ position: "sticky", top: 0, zIndex: 10, background: colors.background, boxShadow: "0 1px 0 " + colors.grey100 }}>
        <Calendar
          transactions={filter === "all" ? storage.transactions : storage.transactions.filter((tx) => tx.type === filter)}
      ...
      <div style={{ flex: 1, padding: "0 18px 120px" }}>
        ...
        {groupedTransactions.length === 0 ? (
          isEmpty ? (
            <div style={{ display: "flex", flexDirection: "column", alignItems: "center", padding: "56px 20px", gap: "8px" }}>
              <Text typography="t6" fontWeight="semibold" color={colors.grey400} style={{ textAlign: "center" }}>
                환전하실 때마다 1분만 기록해보세요
              </Text>
              <Text typography="t7" color={colors.grey300} style={{ textAlign: "center" }}>
                내 달러 평균 환전단가를 쌓아갈 수 있어요
              </Text>
            </div>
          ) : (
      ...
      <BottomSheet
        ...
        header={<BottomSheet.Header>환전 유형</BottomSheet.Header>}
      >
        <BottomSheet.Select
